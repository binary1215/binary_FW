...
class CocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, image_dir, ann_file, *,
        ...
        self.ids = self.coco.getImgIds(imgIds=self.coco.imgs.keys(),catIds=self.category_ids)
    ...
    def filter_for_annotations(self, *, min_kp_anns=0):
        ...
        def filter_image(image_id):
            ...
            kp_anns = [ann for ann in anns
                       if 'keypoints' in ann and np.any(np.array(v) > 0.0 for v in ann['keypoints'][2::3])]
    ...
    def __getitem__(self, index):
        ...
        if self.with_raw:
            raw_img,spare_ann = copy.deepcopy(image), copy.deepcopy(anns)
            image, anns, meta = self.preprocess(image, anns, meta)
            # LOG.debug(meta)
            return raw_img,image, [anns,spare_ann], meta

        image, anns, meta = self.preprocess(image, anns, meta)
        # anns[0] = torch.nan_to_num(anns[0])
        # anns[1] = torch.nan_to_num(anns[1])
        # np_image = image.numpy()
        # np_fn =  'F:/python_project/falldetection_openpifpaf-master/check_npy/' + image_info.split('.')[0] + '.npy'
        # if os.path.exists(np_fn) is False:
        #     np.save(np_fn, np_image)
        # np_anns_1 = anns[0].numpy()
        # np_anns_2 = anns[1].numpy()
        # np_fn_1 = 'F:/python_project/falldetection_openpifpaf-master/check_npy/' + image_info.split('.')[0] + '_1_1.npy'
        # np_fn_2 = 'F:/python_project/falldetection_openpifpaf-master/check_npy/' + image_info.split('.')[0] + '_2_1.npy'
        # if os.path.exists(np_fn_1) is False:
        #     np.save(np_fn_1, np_anns_1)
        #     np.save(np_fn_2, np_anns_2)
        ...
        # np_anns_1 = anns[0].numpy()
        # np_anns_2 = anns[1].numpy()
        # np_fn_1 = 'F:/python_project/falldetection_openpifpaf-master/check_npy/' + image_info.split('.')[0] + '_1_2.npy'
        # np_fn_2 = 'F:/python_project/falldetection_openpifpaf-master/check_npy/' + image_info.split('.')[0] + '_2_2.npy'
        # # if os.path.exists(np_fn_1) is False:
        # #     np.save(np_fn_1, np_anns_1)
        # #     np.save(np_fn_2, np_anns_2)
        #
